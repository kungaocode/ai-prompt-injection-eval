"""
Training Health Monitor — run periodically to check that training
...
OUTPUT_DIR = r"E:\model\Qwen3-4B-Base-finetuned"
...
def check_latest_checkpoint():
    ...
    if last_train:
        ...
        gn_v = last_train.get('grad_norm')
        loss_str = f"{loss_v:.4f}" if loss_v is not None else "N/A"
        lr_str = f"{lr_v:.2e}" if lr_v is not None else "N/A"
        gn_str = f"{gn_v:.4f}" if gn_v is not None else "N/A"
        print(f"   train_loss={loss_str}, lr={lr_str}, grad_norm={gn_str}")
    if last_eval:
